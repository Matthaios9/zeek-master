...
void TCPSessionAdapter::DeliverPacket(int len, const u_char* data, bool is_orig, uint64_t seq, const IP_Hdr* ip,
    ...
    for ( auto i = packet_children.begin(); i != packet_children.end();  ) {
        auto child = *i;
...
void TCPSessionAdapter::AttemptTimer(double ) {
    if ( ! is_active )
        return;
...
void TCPSessionAdapter::PartialCloseTimer(double ) {
    if ( ! is_active )
        return;
...
void TCPSessionAdapter::ResetTimer(double ) {
    if ( ! is_active )
        return;
...
void TCPSessionAdapter::DeleteTimer(double ) { session_mgr->Remove(Conn()); }
