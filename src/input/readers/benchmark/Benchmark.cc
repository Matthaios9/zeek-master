...
bool Benchmark::DoUpdate() {
    ...
    for ( int i = 0; i < linestosend; i++ ) {
        ...
        if ( timedspread != 0.0 ) {
            ...
            while ( diff / heartbeat_interval < i / (linestosend + (linestosend * timedspread)) );
        }
    }
...
threading::Value* Benchmark::EntryToVal(TypeTag type, TypeTag subtype) {
    ...
    switch ( type ) {
        ...
        case TYPE_TIME: val->val.double_val = CurrTime(); break;

        case TYPE_DOUBLE:
        case TYPE_INTERVAL: val->val.double_val = random(); break;

        case TYPE_COUNT: val->val.uint_val = random(); break;
