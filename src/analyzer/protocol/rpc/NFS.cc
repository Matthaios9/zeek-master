...
bool NFS_Interp::RPC_BuildCall(RPC_CallInfo* c, const u_char*& buf, int& n) {
    ...
    switch ( proc ) {
        case BifEnum::NFS3::PROC_NULL: break;

        case BifEnum::NFS3::PROC_GETATTR: callarg = nfs3_fh(buf, n); break;
        ...
        case BifEnum::NFS3::PROC_READLINK: callarg = nfs3_fh(buf, n); break;

        case BifEnum::NFS3::PROC_SYMLINK: callarg = nfs3_symlinkargs(buf, n); break;

        case BifEnum::NFS3::PROC_LINK: callarg = nfs3_linkargs(buf, n); break;
        ...
        case BifEnum::NFS3::PROC_READDIR: callarg = nfs3_readdirargs(false, buf, n); break;

        case BifEnum::NFS3::PROC_READDIRPLUS: callarg = nfs3_readdirargs(true, buf, n); break;
...
bool NFS_Interp::RPC_BuildReply(RPC_CallInfo* c, BifEnum::rpc_status rpc_status, const u_char*& buf, int& n,
    ...
    switch ( c->Proc() ) {
        case BifEnum::NFS3::PROC_NULL: event = nfs_proc_null; break;
...
RecordValPtr NFS_Interp::nfs3_readdir_reply(bool isplus, const u_char*& buf, int& n, BifEnum::NFS3::status_t status) {
...
uint32_t NFS_Interp::ExtractUint32(const u_char*& buf, int& n) { return extract_XDR_uint32(buf, n); }

uint64_t NFS_Interp::ExtractUint64(const u_char*& buf, int& n) { return extract_XDR_uint64(buf, n); }
