    ...
    if ( f ) {
        if ( frame->is_orig() )
            analyzer->EnqueueConnEvent(f, analyzer->ConnVal(), val_mgr->Count(frame->frame_type()),
                                       val_mgr->Count(frame->body_length()), val_mgr->Count(req_func));
        else
            analyzer->EnqueueConnEvent(f, analyzer->ConnVal(), val_mgr->Count(frame->frame_type()),
                                       val_mgr->Count(frame->body_length()), val_mgr->Count(req_frame_type),
                                       val_mgr->Count(req_func), val_mgr->Count(frame->reply()->completion_code()));
    }
}
...
void Contents_NCP_Analyzer::DeliverStream(int len, const u_char* data, bool orig) {
    ...
    if ( ! resync_set ) {
        ...
        if ( tcp )
            resync = (IsOrig() ? tcp->OrigState() : tcp->RespState()) != analyzer::tcp::TCP_ENDPOINT_ESTABLISHED;
        else
            resync = false;
    }
