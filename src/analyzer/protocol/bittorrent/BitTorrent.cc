...
BitTorrent_Analyzer::BitTorrent_Analyzer(Connection* c) : analyzer::tcp::TCP_ApplicationAnalyzer("BITTORRENT", c) {
    interp = new binpac::BitTorrent::BitTorrent_Conn(this);
    stop_orig = stop_resp = false;
    stream_len_orig = stream_len_resp = 0;
}

BitTorrent_Analyzer::~BitTorrent_Analyzer() { delete interp; }
...
void BitTorrent_Analyzer::DeliverWeird(const char* msg, bool orig) {
    if ( bittorrent_peer_weird )
        ...
        EnqueueConnEvent(bittorrent_peer_weird, ConnVal(), val_mgr->Bool(orig), make_intrusive<StringVal>(msg));
}
