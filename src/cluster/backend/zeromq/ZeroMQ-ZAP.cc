...
void zap_thread_fun(ZapArgs* zap_args) {
    ...
    while ( ! done ) {
        ...
        try {
            ...
            if ( r && *r >= 7 ) {
                request_id = request_frames[1].to_string_view();
            ...
            response_frames.emplace_back(version.data(), version.size());
            response_frames.emplace_back(request_id.data(), request_id.size());
